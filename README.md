...
### 3. **Compute kGain Score:**
   ...
   - For each k-mer, calculate the fold change between the genomic frequencies of the k-mers containing the alternate allele (F^alt(i(v))) $F^{\text{alt}}_{i(v)}$ and the reference allele  $F^{\text{Ref}}_{i(v)}$ (F^Ref(i(v))).
