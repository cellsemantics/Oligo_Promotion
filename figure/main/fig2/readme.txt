...
This folder contains a single notebook, `fig2.ipynb`, that builds the Figure 2 analyses and panels.

This README was updated to match the notebook logic and currently saved outputs.
...
- labels mutations by mutator class, gene essentiality, fixation status, and selection class,
- generates figure panels (2e–2m),
- and runs the logistic regression used in Fig. 2m.
...
| Heatmap generation `bin_size` | `2500` |
| Bootstrap CI seed (effect size) | `42` |
| Bootstrap iterations for effect-size CI | `2000` (default in `utility.median_based_distance_ci`) |

Mutator grouping used in the notebook:
...
2. **Annotation merges**
   - Adds mutator/non-mutator label.
   - Adds essentiality labels.
   - Adds allele-frequency trajectories.

3. **Fixation and selection labels**
   - `fixed` if final `min_last_points` allele frequencies are all `>= 0.95`; otherwise `not_fixed`.
   - Fits per-mutation slope via `linregress(logit(allele_freq) ~ generation_number)` when at least 10 generation points exist.
   - BH/FDR correction on mutation-wise raw p-values.
   - Classification:
...
### Fig. 2e
- Displays `classification` counts.
...
- Pairwise test in notebook:
  - **One-sided** Mann–Whitney (`utility.man_whiteney(..., alternative="greater")`)
  - BH/FDR correction done **within each family** (`mutator` and `non_mutator`, 3 tests each)
  - Effect size + 95% CI from `utility.median_based_distance_ci`
- Current printed stats:
...
- Uses `bin_size = 2500`, `BrBG_r`, and fixed population order:
...
### Fig. 2m
- Logistic regression: beneficial (`positive`) vs neutral/negative.
- Features:
  - `evolved_kGain`
...
## Dependencies used by this notebook
...
- Project-root discovery is dynamic (walks up to find `utility.py`), so execution does not depend on Jupyter launch path.
- Fig. 2f/2g/2h use `r_plot_utils.plot_custom_bar_r(...)`, so R support is required for those cells.
