...
def box_plot(data, x, y, ax, hue = None):
    ...
    sns.boxplot(x=x, y=y, data=data, hue=hue, boxprops=dict(edgecolor="black", linewidth=0),ax=ax,flierprops=dict(marker='o', markeredgecolor='black'), width=0.4,linewidth=0.5, fliersize=0.15, dodge=True, order=median_per_category.index, medianprops={"color": "black","linewidth":1}, color= "lime")
    return ax
